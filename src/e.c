...
#define ΨX(X, ...)                                                             \
  X = (s_t *)(const void *[]){0, 0, __VA_ARGS__, ο, α, ρ},                     \
  X[0].Q = NUMARGS(__VA_ARGS__) + 5
...
#define D LOG, O
#define C(α)                                                                   \
  α[1].v = σ, σ = α, ο = σ[σ[0].Q - 3].v, α = σ[σ[0].Q - 2].v,                 \
  ρ = σ[σ[0].Q - 1].v, ι = σ[0].Q - 3, D
void nar(OARS) { C(α); }
void e2(OARS) { LOG; }
...
void add_(OARS) {
  Ψ(os)(σ[1].s[2].Q + σ[1].s[1].s[2].Q, ob)(os);
  C(α);
}
