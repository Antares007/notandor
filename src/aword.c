...
void word(obarts, void *wordcore) {
  s_t *Tp = T(propeller, wordcore);
  t[a - 1].c(O(T(Tp, T(cr0), wrap),     //
               T(Tp, T(cr1), wrap),     //
               T(Tp, T(cr2), wrap),     //
               T(Tp, T(cr3), wrap), o), //
             b, a - 1, r, t, s);
...
N(Nat_o) {
  t[a - 1].c(O(T(dig03), T(dig47), T("8", match), T("9", match), o), b, a - 1,
             r, t, s);
}
...
N(Nats);
N(Nats_o) {
  t[a - 1].c(O(T(Nat), T(Nats, Nat), T(ε), T(ε), o), b, a - 1, r, t, s);
}
